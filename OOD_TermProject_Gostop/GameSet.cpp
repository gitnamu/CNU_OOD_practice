...
Card* Card48 = builder.setType(4)
                   ...
                   .build();
Card* Card49 =
    builder.setType(4).setMonth(0).setSsangP().setName("Bonus 쌍피").build();
Card* Card50 =
    builder.setType(4).setMonth(0).setSsangP().setName("Bonus 쌍피").build();
Card cardset[50] = {
...
std::vector<Card*>* GameSet::GetFloor() { return floor; }
std::stack<Card*>* GameSet::GetDeck() { return deck; }
GameSet::GameSet()
    : deck(new std::stack<Card*>), floor(new std::vector<Card*>) {}

void GameSet::SetPlayerHand(Player* p1, Player* p2,
                            Player* p3) {  // 플레이어들에게 7장씩 배분
  for (int i = 0; i < 7; i++) {
    p1->setHandField(this->GetDeck()->top());
    this->GetDeck()->pop();
    p2->setHandField(this->GetDeck()->top());
    this->GetDeck()->pop();
    p3->setHandField(this->GetDeck()->top());
    this->GetDeck()->pop();
  }
}
...
void GameSet::PairCheck(Player* turn, Card* card, Player* other1,
...
void GameSet::Run(Player* turn, Player* other1, Player* other2) {
  ...
  if (!turn->handField()->empty()) {  // 손패가 비지 않았다면
    ...
    while (handout->cardMonth() == 0) {
      turn->addScoreField(handout);
      std::cout << "보너스 카드를 냈습니다. ! 카드를 뒤집어 손으로 가져옵니다." << std::endl;
      turn->setHandField(deck->top());
      deck->pop();
      turn->printMyHandField();
      handout = turn->handOut();
      fliped = deck->top();
      deck->pop();
    }
...
void GameSet::BbuckShuffle() {  // 뻑 검사를 위한 테스트용 함수. 카드가 무조건
                             // 정순으로 섞임.
  bool test[decksize];
  for (int i = 0; i < decksize; i++) test[i] = false;
  int arr[decksize];
